...
app.get('/exponential', (req, res) => {
    const {num1, num2} = req.query;
    if (!isValidNumber(num1, num2)) return res.status(400).send('<h1>Error: Invalid input. Provide two valid numbers.</h1>');
    res.send(`<h1>Result: ${Math.pow(parseFloat(num1), parseFloat(num2))}</h1>`);
})

app.get('/sqrt', (req, res) => {
    const { num } = req.query;
    if (isNaN(num) || num < 0) return res.status(400).send('<h1>Error: Invalid input. Provide a non-negative number.</h1>');
    res.send(`<h1>Result: ${Math.sqrt(parseFloat(num))}</h1>`);
});
